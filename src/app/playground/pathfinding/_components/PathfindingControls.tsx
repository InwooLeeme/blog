...
export default memo(function PathfindingControls({ algorithm, tool, running, dispatch }: {
  ...
  return (
    <section aria-label="탐색 설정" className="border-b border-border px-4 py-4 sm:px-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <fieldset disabled={running} className="flex min-w-0 flex-wrap items-center gap-3">
          ...
          <div className="inline-grid grid-cols-3 gap-1" role="group" aria-label="알고리즘 선택">
            {ALGORITHMS.map(({ id, label }) => (
              <button key={id} type="button" aria-pressed={algorithm === id} onClick={() => dispatch({ type: "algorithm", value: id })}
                className={`min-h-11 rounded-md px-3 text-sm font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-brand disabled:cursor-not-allowed sm:px-5 ${algorithm === id ? "bg-muted text-foreground" : "text-muted-foreground hover:bg-muted/60"}`}>{label}</button>
            ))}
          </div>
        </fieldset>
          ...
          <div className="flex flex-wrap gap-2">
            {TOOLS.map(({ id, label, icon: Icon }) => (
              <button key={id} type="button" aria-pressed={tool === id} onClick={() => dispatch({ type: "tool", value: id })}
                className={`${controlClass} ${tool === id ? "border-border bg-muted text-foreground" : "border-transparent text-muted-foreground"}`}><Icon size={15} aria-hidden="true" />{label}</button>
            ))}
          </div>
        </fieldset>
      ...
      <p className="mt-3 text-xs leading-5 text-muted-foreground">{running ? "탐색 중에는 지도가 잠깁니다. 일시정지 후 편집할 수 있습니다." : "지도를 편집하면 이전 탐색과 비교 결과가 지워집니다."}</p>
    </section>
  );
});
