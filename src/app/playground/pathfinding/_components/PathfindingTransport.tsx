...
import type { PlaygroundAction, PlaygroundState, Speed } from "../_lib/playground-state";
import { controlClass } from "./PathfindingControls";

export default function PathfindingTransport({ state, dispatch, onPlay }: {
  ...
  return (
    <section aria-label="탐색 재생" className="flex flex-wrap items-center gap-3 border-y border-border bg-muted/20 px-4 py-4 sm:px-6">
      ...
      <div className="flex grow flex-wrap items-center gap-3 sm:justify-end">
        ...
        <select id="pathfinding-speed" value={state.speed} onChange={(event) => dispatch({ type: "speed", value: event.target.value as Speed })} className="min-h-11 grow rounded-lg border border-border bg-background px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-brand sm:grow-0">
